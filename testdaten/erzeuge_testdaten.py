...
Ausgaben (im Ordner testdaten/):
  ...
  folgejahr/EXTF_Buchungsstapel_2026_Demo.csv  Folgejahres-Stapel 01/2026
      (CO-02; eigener Unterordner, damit --stapel testdaten den
      Folgejahres-Stapel NICHT als Pruefjahr einsammelt)
...
# --- Volumen-Fuellbuchungen: realistische Kleinst-KapG (>= 1.200 Zeilen) -----
# Ziel: genug Buchungsmasse, damit die Statistik-Ebene wirklich greift
# (ST-08 Benford ab benford_min_n GuV-Buchungen), ohne EINEN gesaeten
# Befund zu veraendern. Konstruktionsregeln (befundneutral):
#   - eigene, beschriftete Konten (8401/3405/4985/4970, Debitor 10010,
#     Kreditor 70010) - gesaete Konten/Serien bleiben unberuehrt;
#   - Erstziffern per Restbedarfs-Ziehung gegen das Benford-Sollprofil
#     des GESAMTbestands -> ST-08 laeuft aktiv und bleibt befundfrei;
#   - Betragsfenster [3B, 30B) je Strom (Spannweite 10:1) haelt den
#     robusten MAD-z unter ~3 -> keine ST-02-Scheinausreisser;
#   - eigener lueckenloser Rechnungskreis FA2025-#### mit datumsmonotoner
#     Vergabe (RE-01/02/03-neutral); Zahlungen im Debitorenkreis tragen
#     denselben Beleg (haelt die RE-Parsequote ueber 60 %);
#   - Kreditoren-Zahlungen und Bankgebuehren beleglos (Bankimport-
#     Realitaet; haelt die gewollte ST-05-Belegfeldquote ueber 20 %);
#   - Zahlungen stets > doppel_fenster_tage nach Rechnung (ST-01-neutral),
#     alle Posten bis 30.12. ausgeglichen (OPOS-/OP-Salden-neutral);
#   - gleichmaessige Monats-/Tagesverteilung (GV-01-neutral), Betraege
#     unter cutoff_min_eur (CO-01-neutral) und unter llm_kandidat_min_eur
#     auf sachfremd-Konten (4985/4970), Cents variabel (FR-04-neutral).

_BENFORD = (0.30103, 0.17609, 0.12494, 0.09691, 0.07918,
            0.06695, 0.05799, 0.05115, 0.04576)


def _erstziffer(betrag: Decimal) -> int:
    for z in f"{betrag:f}":
        if z in "123456789":
            return int(z)
    return 0
...
def _guv_ziffernprofil() -> list[int]:
    """Erstziffern-Histogramm der bisherigen GuV-Buchungen (SKR03:
    2000-4999/8000-8999, ohne Saldovortragszeilen) - Naeherung der
    ST-08-Basis fuer die Restbedarfs-Ziehung."""
    profil = [0] * 10
    for umsatz, _sh, konto, gegen, _bu, _d, _bg, _t in BUCHUNGEN:
        if 9000 <= konto <= 9099 or 9000 <= gegen <= 9099:
            continue
        if 2000 <= konto <= 8999 or 2000 <= gegen <= 8999:
            profil[_erstziffer(umsatz)] += 1
    return profil


def _benford_ziffernplan(n: int, rng: random.Random) -> list[int]:
    profil = _guv_ziffernprofil()
    gesamt = sum(profil[1:]) + n
    bedarf = {d: max(0, round(_BENFORD[d - 1] * gesamt) - profil[d])
              for d in range(1, 10)}
    plan: list[int] = []
    while len(plan) < n:
        d = max(bedarf, key=lambda z: (bedarf[z], -z))
        if bedarf[d] > 0:
            bedarf[d] -= 1
        else:
            d = rng.choices(range(1, 10), weights=_BENFORD)[0]
        plan.append(d)
    rng.shuffle(plan)
    return plan


def _betrag(ziffer: int, basis: int, rng: random.Random) -> Decimal:
    """Betrag mit vorgegebener Erstziffer im Fenster [3*basis, 30*basis):
    Ziffern 1-2 liegen in der oberen, 3-9 in der unteren Dekade."""
    dekade = basis * 10 if ziffer <= 2 else basis
    euro = int((ziffer + rng.random()) * dekade)
    return D(euro) + D(rng.randrange(100)) / 100


def _jahrestag(i: int, n: int) -> date:
    """Datumsmonoton uebers Jahr verteilt (Jahresende ausgespart, damit
    Zahlungen im Jahr bleiben und das CO-01-Fenster frei bleibt)."""
    return date(2025, 1, 2) + timedelta(days=(i * 348) // n)


_rng = random.Random(20250813)
_N_AR, _N_ER, _N_KG, _N_GB = 340, 160, 170, 120
_plan = _benford_ziffernplan(_N_AR + _N_ER + _N_KG + _N_GB, _rng)
_ar = _plan[:_N_AR]
_er = _plan[_N_AR:_N_AR + _N_ER]
_kg = _plan[_N_AR + _N_ER:_N_AR + _N_ER + _N_KG]
_gb = _plan[_N_AR + _N_ER + _N_KG:]

# Dienstleistungserloese: Rechnung Debitor an 8401 (Automatik 19 %),
# Zahlungseingang mit Beleg nach 8-20 Tagen
for i, ziffer in enumerate(_ar):
    tag = _jahrestag(i, _N_AR)
    betrag = _betrag(ziffer, 100, _rng)              # 300,00-2.999,99 EUR
    buche(betrag, "S", 10010, 8401, tag,
          f"Ausgangsrechnung Dienstleistung {tag.month:02d}/2025",
          beleg=f"FA2025-{1001 + i}")
    zahltag = min(tag + timedelta(days=8 + _rng.randrange(13)),
                  date(2025, 12, 30))
    buche(betrag, "S", 1200, 10010, zahltag, "Zahlungseingang",
          beleg=f"FA2025-{1001 + i}")

# Handelswaren-Einkauf: 3405 (Automatik VSt 19 %) an Kreditor,
# beleglose Bankzahlung nach 8-20 Tagen
for i, ziffer in enumerate(_er):
    tag = _jahrestag(i, _N_ER)
    betrag = _betrag(ziffer, 40, _rng)               # 120,00-1.199,99 EUR
    buche(betrag, "S", 3405, 70010, tag,
          f"Wareneingang Handelsware {tag.month:02d}/2025",
          beleg=f"WE2025-{2001 + i}")
    zahltag = min(tag + timedelta(days=8 + _rng.randrange(13)),
                  date(2025, 12, 30))
    buche(betrag, "S", 70010, 1200, zahltag, "Zahlung Sigma Großhandel")

# Kleingeraete/Verbrauchsmaterial direkt per Bank (VSt-Schluessel 9)
for i, ziffer in enumerate(_kg):
    tag = _jahrestag(i, _N_KG)
    betrag = _betrag(ziffer, 8, _rng)                # 24,00-239,99 EUR
    buche(betrag, "S", 4985, 1200, tag,
          f"Kleingeräte/Verbrauchsmaterial {tag.month:02d}/2025",
          bu="9", beleg=f"WZ2025-{3001 + i}")

# Nebenkosten des Geldverkehrs: beleglose, schluesselfreie Bankgebuehren
for i, ziffer in enumerate(_gb):
    tag = _jahrestag(i, _N_GB)
    betrag = _betrag(ziffer, 4, _rng)                # 12,00-119,99 EUR
    buche(betrag, "S", 4970, 1200, tag,
          f"Kontofuehrung/Entgelte {tag.month:02d}/2025")


# --- Folgejahres-Stapel Januar 2026 (nur Datenquelle fuer CO-02) ------------
# CO-02: genau EIN Treffer (ER-2026-015, nachlaufende Eingangsrechnung mit
# Leistungsbezug Dezember). Kontrollfaelle ohne Treffer: EB-Buchung
# (Saldovortrag), Miete Januar (Dauerbuchungs-Muster des Pruefjahres,
# zudem unter cutoff_min_eur), Kleinbetrag unter cutoff_min_eur,
# Erloesbuchung (CO-02 prueft nur die Aufwandsseite), Gehaelter 28.01.
# (ausserhalb des 14-Tage-Fensters).
FJ_BUCHUNGEN: list[tuple] = []


def buche_fj(umsatz, sh, konto, gegen, datum, text, bu="", beleg=""):
    FJ_BUCHUNGEN.append((D(str(umsatz)), sh, konto, gegen, bu, datum, beleg, text))


buche_fj(45000, "S", 1200, 9000, date(2026, 1, 1), "EB Bank")
buche_fj(2000, "S", 4210, 1200, date(2026, 1, 1), "Miete Büro 01/2026")
buche_fj(6500, "S", 3400, 70001, date(2026, 1, 8),
         "Wareneinkauf Lieferung 29.12.2025", bu="9", beleg="ER-2026-015")  # CO-02
buche_fj(120, "S", 4930, 1000, date(2026, 1, 9), "Büromaterial bar",
         beleg="BAR-2026-01")
buche_fj(7140, "S", 10001, 8400, date(2026, 1, 10), "Ausgangsrechnung 01/2026",
         beleg="RE2026-001")
buche_fj(6500, "S", 4120, 1200, date(2026, 1, 28), "Gehälter 01/2026")
...
def _schreibe_stapeldatei(dateiname: str, kopf1: str, buchungen: list) -> Path:
...
def schreibe_stapel() -> Path:
    kopf1 = ('"EXTF";700;21;"Buchungsstapel";12;20260812090000000;;"RE";"";"";'
             ...
             '"EUR";;"";;;"";;"";;""')
    return _schreibe_stapeldatei("EXTF_Buchungsstapel_2025_Demo.csv",
                                 kopf1, BUCHUNGEN)


def schreibe_stapel_folgejahr() -> Path:
    # WJ-Beginn 01.01.2026, Header-Zeitraum Januar 2026 (Teillieferung
    # reicht: CO-02 braucht nur das Fenster ab dem 01.01.)
    kopf1 = ('"EXTF";700;21;"Buchungsstapel";12;20260812090000000;;"RE";"";"";'
             '12345;67890;20260101;4;20260101;20260131;"Demo 2026-01";"MF";1;0;0;'
             '"EUR";;"";;;"";;"";;""')
    (ZIEL / "folgejahr").mkdir(exist_ok=True)
    return _schreibe_stapeldatei("folgejahr/EXTF_Buchungsstapel_2026_Demo.csv",
                                 kopf1, FJ_BUCHUNGEN)
...
KONTONAMEN = {
    ...
    3405: "Wareneingang Handelswaren 19 % VSt",
    4120: "Gehälter",
    ...
    4930: "Bürobedarf", 4970: "Nebenkosten des Geldverkehrs",
    4980: "Betriebsbedarf", 4985: "Werkzeuge und Kleingeräte",
    8100: "Steuerfreie Umsätze § 4 UStG", 8400: "Erlöse 19 % USt",
    8401: "Erlöse Dienstleistungen 19 % USt",
    ...
    10010: "Kunde Sammelkonto Laufkundschaft",
    70001: "Lieferant Delta GmbH", 70002: "Lieferant Epsilon e.K.",
    70003: "Werbeagentur Zeta", 70010: "Lieferant Sigma Großhandel GmbH",
}
...
def schreibe_susa_vorjahr() -> Path:
    ...
    Konto 868 schliesst die SuSa doppisch auf null (eine echte SuSa
    summiert immer auf 0); ohne EB-Uebernahme im Stapel -> gewollter
    zusaetzlicher VJ-01-Hinweis (kapitalnah), konsistent zur
    SB-06-Story unvollstaendiger Vortraege."""
    ...
    vj[4640] = D("3000.00")
    vj.pop(4800, None)
    vj[868] = -sum(vj.values())  # Ergebnisvortrag als EK-Gegenwert
    zeilen = ["Konto;Saldo"]
...
if __name__ == "__main__":
    ...
    print(f"{len(BUCHUNGEN)} Buchungszeilen (+{len(FJ_BUCHUNGEN)} Folgejahr) | "
          f"Kasse: Minimum {minimum}, Endstand {ende} "
          "(erwartet: Minimum < 0, Endstand >= 0)")
    for pfad in (schreibe_stapel(), schreibe_stapel_folgejahr(),
                 schreibe_kontenbeschriftungen(), schreibe_susa(),
                 schreibe_susa_vorjahr(), schreibe_opos()):
