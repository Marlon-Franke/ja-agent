...
from befunde import QUELLEN_BASIS, Befund, Kontext, eur
...
def _b(ctx: Kontext, cid: str, schwere: str, text: str, empfehlung: str = "",
    ...
    if not quelle:
        quelle = QUELLEN_BASIS.get(cid, "Buchungsstapel")
        if konto is not None:
            quelle += f", Konto {konto}"
...
_ERWARTUNG = [  # (Gruppe, erwartetes Vorzeichen des Saldos, netto verwenden)
    ...
    ("rap_aktiv", 1, False), ("rap_passiv", -1, False),
...
def us_04_bewirtung(ctx: Kontext) -> list[Befund]:
    ...
    if bewirtung > 0 and nabz > 0:
        quote = nabz / (bewirtung + nabz)
        quote_min = Decimal(str(ctx.param.get("bewirtung_nabz_quote_min", "0.2")))
        if quote < quote_min:
            return [_b(ctx, "US-04", "hinweis",
                f"Nicht abziehbarer Bewirtungsanteil {eur(nabz)} EUR entspricht "
                f"nur {float(quote):.0%} des Bewirtungsvolumens – erwartet "
                "werden 30 % (§ 4 Abs. 5 Nr. 2 EStG).",
                "Aufteilungsquote prüfen; zu 100 % abziehbare Arbeitnehmer-"
                "bewirtung mindert die Quote zulässig – dann auf eigenem "
                "Konto führen.", betrag=nabz)]
    return []
...
def us_06_07_verprobung(ctx: Kontext) -> list[Befund]:
    ...
    ctx.ust_rechnerisch, ctx.vst_rechnerisch = ust, vst
    if ctx.susa is None:
...
def _ist_kapitalgesellschaft(ctx: Kontext) -> bool:
    return str(ctx.param.get("rechtsform", "")).lower() == "kapitalgesellschaft"


def pp_01_kfz_privat(ctx: Kontext) -> list[Befund]:
    if _ist_kapitalgesellschaft(ctx):
        ctx.skip("PP-01", "Rechtsform Kapitalgesellschaft: Privatnutzung "
                          "läuft über Lohn/vGA (erfordert Lohnjournal)")
        return []
...
def pp_02_privat_jahresende(ctx: Kontext) -> list[Befund]:
    if _ist_kapitalgesellschaft(ctx):
        ctx.skip("PP-02", "Rechtsform Kapitalgesellschaft: Privatkonten "
                          "unzulässig – siehe PP-04")
        return []
    if ctx.wj_ende is None:
        return []
    fenster = ctx.wj_ende - timedelta(days=int(ctx.param["cutoff_fenster_vor_tage"]))
    ...
    if len(daten) >= 3 and min(daten) > fenster:
        return [_b(ctx, "PP-02", "hinweis",
            f"Alle {len(daten)} Privatbuchungen liegen im Jahresend-Fenster "
            f"{fenster + timedelta(days=1):%d.%m.%Y}–{ctx.wj_ende:%d.%m.%Y} "
            f"(Parameter 'cutoff_fenster_vor_tage').",
