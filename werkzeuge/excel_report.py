...
JOURNAL_SPALTEN = [
    ("Datei", 24), ("Zeile", 7), ("Datum", 11), ("Umsatz (EUR)", 13),
    ("S/H", 5), ("Storno", 7), ("Konto", 9), ("Kontobezeichnung", 22),
    ("Gegenkonto", 11), ("Gegenkonto-Bezeichnung", 22), ("BU", 6),
    ("Beleg", 13), ("Buchungstext", 32), ("KOST1", 9), ("EB", 5),
]


def journal_zeilen(ctx: Kontext) -> dict[str, int]:
    """Blattzeile je Stapelzeile im »Buchungsjournal« (Schlüssel
    Datei:Zeilennummer – dasselbe Format wie die Befund-Spalte Quelle)."""
    return {f"{b.quelle}:{b.zeile}": i
            for i, b in enumerate(ctx.buchungen, start=2)}


def _quelle_link(zelle, quelle: str, journal: dict[str, int]) -> None:
    """Verlinkt eine Quelle-Zelle auf die Journalzeile, wenn die Quelle
    buchungsgenau ist (letztes Segment = Datei:Zeilennummer)."""
    schluessel = quelle.split(" / ")[-1] if quelle else ""
    ziel = journal.get(schluessel)
    if ziel:
        zelle.hyperlink = Hyperlink(ref=zelle.coordinate,
                                    location=f"'Buchungsjournal'!A{ziel}")
...
def _befundblatt(wb: Workbook, titel: str, befunde: list[Befund],
                 ctx: Kontext, journal: dict[str, int]) -> None:
    ...
    for f in sorted(befunde, key=lambda f: (RANG[f.schwere], f.check_id)):
        ...
        _quelle_link(_zelle(ws, zeile, 15, f.quelle), f.quelle, journal)
...
_ANLEITUNG = [
    ...
    ("Spalten der Befundblätter", [
        ...
        "als Hyperlink direkt auf die Zeile im Blatt »Buchungsjournal« "
        "(alle Buchungszeilen des Stapels, per AutoFilter durchsuchbar); "
        "bei konten- oder abgleichsbasierten Befunden die Datenbasis des "
        "Checks (z. B. »OPOS-Liste ↔ Buchungsstapel«, »SuSa ↔ "
        "Buchungsstapel«, »Vorjahres-SuSa ↔ EB-Buchungen«) mit "
        "Kontonummer.",
...
def _kandidatenblatt(wb: Workbook, kandidaten: list[dict], ctx: Kontext,
    ...
    for zeile, k in enumerate(kandidaten, start=2):
        ...
        _quelle_link(_zelle(ws, zeile, 11, k["quelle"]), k["quelle"], journal)


def _journalblatt(wb: Workbook, ctx: Kontext) -> None:
    """Alle Buchungszeilen des Stapels 1:1 (Reihenfolge = Einleseordnung,
    dadurch stimmen die Zeilennummern aus journal_zeilen); Ziel der
    Quelle-Hyperlinks aus Befund- und Kandidatenblättern."""
    ws = wb.create_sheet("Buchungsjournal")
    _kopfzeile(ws, JOURNAL_SPALTEN)
    for zeile, b in enumerate(ctx.buchungen, start=2):
        _zelle(ws, zeile, 1, b.quelle)
        _zelle(ws, zeile, 2, b.zeile)
        _zelle(ws, zeile, 3, b.belegdatum, fmt="DD.MM.YYYY")
        _zelle(ws, zeile, 4, float(b.umsatz), fmt="#,##0.00")
        _zelle(ws, zeile, 5, b.sh)
        _zelle(ws, zeile, 6, "ja" if b.storno else "")
        _zelle(ws, zeile, 7, b.konto)
        _zelle(ws, zeile, 8, ctx.name(b.konto))
        _zelle(ws, zeile, 9, b.gegenkonto)
        _zelle(ws, zeile, 10, ctx.name(b.gegenkonto))
        _zelle(ws, zeile, 11, b.bu)
        _zelle(ws, zeile, 12, b.belegfeld1)
        _zelle(ws, zeile, 13, b.buchungstext)
        _zelle(ws, zeile, 14, b.kost1)
        _zelle(ws, zeile, 15, "ja" if ctx.ist_eb(b) else "")


# Public: auch Basis der OPOS-Stufen in opos.csv (ja_pruefung.py)
AGING_STUFEN = [(0, "nicht fällig"), (30, "0–30 Tage"), (60, "31–60 Tage"),
                (90, "61–90 Tage"), (180, "91–180 Tage"),
                (365, "181–365 Tage"), (99999, "> 365 Tage")]


def aging_stufe(tage: int) -> str:
    if tage < 0:
        return "nicht fällig"
    for grenze, label in AGING_STUFEN[1:]:
        if tage <= grenze:
            return label
    return "> 365 Tage"
...
def _opos_aging(wb: Workbook, ctx: Kontext) -> None:
    ...
    labels = [label for _g, label in AGING_STUFEN]
    ...
    for p in ctx.opos:
        ...
        stufe = aging_stufe((ctx.datum_bis - datum).days) if datum else "0–30 Tage"
...
def _saldenblatt(wb: Workbook, ctx: Kontext) -> None:
    ...
    for k in sorted(ctx.anzahl):
        ...
        zeile += 1
    if ctx.susa_vj is not None:
        # Vorjahreskonten ohne Berichtsjahresbewegung (Spalten 8/9/10 =
        # Vorjahressaldo, Buchungen 0, Position nach VJ-Saldenlage)
        for k in sorted(set(ctx.susa_vj) - set(ctx.anzahl)):
            vj = ctx.susa_vj[k]
            _zelle(ws, zeile, 1, k)
            _zelle(ws, zeile, 2, ctx.name(k), wrap=True)
            _zelle(ws, zeile, 3, ", ".join(ctx.plan.gruppen_von(k)), wrap=True)
            _zelle(ws, zeile, 8, float(vj), fmt="#,##0.00")
            _zelle(ws, zeile, 9, 0)
            _zelle(ws, zeile, 10, bilanz.position(ctx, k, saldo=vj), wrap=True)
            zeile += 1
    ...
    _zelle(ws, zeile + 1, 1,
           ...
           "Bilanzposition aus. Konten mit Vorjahressaldo, aber ohne "
           "Berichtsjahresbewegung stehen mit Buchungen = 0 im Blatt, "
           "damit die Vorjahresspalten von Bilanz und GuV vollständig "
           "sind.",
           wrap=True).font = Font(name=FONT, size=9, italic=True, color="595959")
...
def _bilanz_guv(wb: Workbook, ctx: Kontext) -> None:
    ...
    zeile += 1
    passiv_start = zeile
    for pos in bilanz.passiv_ek(ctx):
        _zelle(ws_b, zeile, 1, pos)
        ...
        if mit_vj:
            _zelle(ws_b, zeile, 3, summe(pos, vj_sp, negiert=True),
                   fmt="#,##0.00")
        zeile += 1
    # A.V direkt in der EK-Gliederung (§ 266 Abs. 3 A. V. HGB; Ausweis
    # vor Ergebnisverwendung, § 268 Abs. 1 HGB) - live aus dem GuV-Blatt
    _zelle(ws_b, zeile, 1, f"{bilanz.ERGEBNIS} (lt. GuV)")
    _zelle(ws_b, zeile, 2, f"=GuV!B{ergebnis_zeile}", fmt="#,##0.00")
    if mit_vj:
        _zelle(ws_b, zeile, 3, f"=GuV!C{ergebnis_zeile}", fmt="#,##0.00")
    zeile += 1
    for pos in bilanz.passiv_uebrig(ctx):
        _zelle(ws_b, zeile, 1, pos)
        _zelle(ws_b, zeile, 2, summe(pos, netto_sp, negiert=True),
               fmt="#,##0.00")
        if mit_vj:
...
def schreibe_bericht(pfad, ctx: Kontext, befunde: list[Befund],
    ...
    wb = Workbook()
    journal = journal_zeilen(ctx)
    _uebersicht(wb.active, ctx, befunde, meta)
    _cockpit(wb, ctx, befunde)
    _befundblatt(wb, "Alle Befunde", befunde, ctx, journal)
    for bereich, blatt in BLATT_NAMEN.items():
        teil = [f for f in befunde if f.bereich == bereich]
        if teil:
            _befundblatt(wb, blatt, teil, ctx, journal)
    _kandidatenblatt(wb, kandidaten, ctx, journal)
    _opos_aging(wb, ctx)
    _bilanz_guv(wb, ctx)
    _saldenblatt(wb, ctx)
    _journalblatt(wb, ctx)
    _anleitung(wb, ctx)
    wb.save(pfad)
