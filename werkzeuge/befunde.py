...
KATALOG: list[tuple[str, str, str, int, str]] = [
    ...
    ("CO-01", "Wesentliche Erlösbuchungen im Cut-off-Fenster vor WJ-Ende", "Cut-off", 3, "P"),
    ("CO-02", "Wesentliche Aufwandsbuchungen im Cut-off-Fenster nach WJ-Ende", "Cut-off", 3, "P/X"),
...
def wj_ende_aus_headern(infos) -> date | None:
    """Wirtschaftsjahr-Ende strikt aus dem DATEV-Header: WJ-Beginn
    (Headerfeld 13) + 12 Monate - 1 Tag; das WJ kann vom Kalenderjahr
    abweichen, 31.12. wird nie unterstellt. Fallback ohne WJ-Beginn:
    spätestes Header-'Datum bis' der Stapel."""
    beginn = [i.wj_beginn for i in infos if i.wj_beginn]
    if beginn:
        b = max(beginn)
        try:
            return date(b.year + 1, b.month, b.day) - timedelta(days=1)
        except ValueError:  # WJ-Beginn 29.02.
            return date(b.year + 1, 3, 1) - timedelta(days=1)
    bis = [i.datum_bis for i in infos if i.datum_bis]
    return max(bis) if bis else None
...
class Kontext:
    ...
    def __init__(self, buchungen: list[Buchung], plan: Kontenplan,
                 ...
                 mandant_name: str = "",
                 infos_folgejahr: list[StapelInfo] | None = None,
                 buchungen_folgejahr: list[Buchung] | None = None):
        ...
        # Anker der Cut-off-/Jahresend-Fenster; bewusst NUR aus den
        # Prüfjahres-Stapeln abgeleitet, damit ein Folgejahres-Stapel
        # den Stichtag nicht verschiebt.
        self.wj_ende: date | None = wj_ende_aus_headern(infos)
        self.infos_folgejahr = infos_folgejahr or []
        self.buchungen_folgejahr = buchungen_folgejahr or []
        self._aggregiere()
