    ...
    for (_betrag, _k, _g), bs in gruppen.items():
        ...
        for a, c in zip(bs, bs[1:]):
            ...
            out.append(_b(ctx, "ST-01", schwere,
                ...
                konto=a.konto, datum=c.belegdatum, betrag=a.umsatz,
                beleg=c.belegfeld1, buchungstext=c.buchungstext,
...
def feiertage(jahr: int) -> set[date]:
    """Bundesweit einheitliche Feiertage (Landesfeiertage nicht enthalten)."""
    o = _ostern(jahr)
    return {date(jahr, 1, 1), o - timedelta(days=2), o + timedelta(days=1),
