...
def baue_llm_kandidaten(ctx: Kontext, befunde) -> list[dict]:
    ...
    geordnet = geordnet[:int(ctx.param["llm_kandidaten_max"])]
    for i, k in enumerate(geordnet, start=1):
        k_id = f"K{i:03d}"
        geordnet[i - 1] = {"id": k_id, **k}
    return geordnet
...
def main(argv=None) -> int:
    ...
    p.add_argument("--kontenbeschriftung", help="Kontenbeschriftungen (EXTF Kat. 20)")
    ...
    befunde = []
    for check in ALLE_FACHLICH + ALLE_ERWEITERT + ALLE_STATISTIK:
        befunde += check(ctx)
    befunde.sort(key=lambda f: (RANG[f.schwere], f.check_id))
    kandidaten = baue_llm_kandidaten(ctx, befunde)
    ...
    print(f"Befunde: {zaehler.get('hoch', 0)} hoch / "
          ...
          f"KI-Kandidaten: {len(kandidaten)}")
