...
def co_01_periodenende(ctx: Kontext) -> list[Befund]:
    ...
    for b in ctx.buchungen:
        ...
        out.append(_b(ctx, "CO-01", "hinweis",
            ...
            f"{b.belegdatum:%d.%m.%Y} – im Jahresend-Fenster "
            f"{ab + timedelta(days=1):%d.%m.%Y}–{ctx.datum_bis:%d.%m.%Y} "
            f"(Parameter 'wj_ende_fenster_tage' = "
            f"{ctx.param['wj_ende_fenster_tage']}).",
