...
def _rechtsform_aus_name(ctx: Kontext, name: str):
    """Klassifiziert den Mandantennamen anhand der konfigurierten
    Rechtsformen-Liste (Parameter 'rechtsformen_erkennung'; Reihenfolge =
    Priorität, zusammengesetzte Formen wie 'GmbH & Co. KG' zuerst)."""
    name = (name or "").lower()
    for regel in ctx.param.get("rechtsformen_erkennung", []):
        if re.search(regel["muster"], name):
            return regel["klasse"]
    return None


def dq_02_rechtsform_konsistenz(ctx: Kontext) -> list[Befund]:
    angabe = str(ctx.param.get("rechtsform", "unbekannt")).lower()
    aus_name = _rechtsform_aus_name(ctx, ctx.mandant_name)
    kapg_ind = sum(ctx.anzahl[k] for k in ctx.konten_in("kst_aufwand")) + \
        sum(ctx.anzahl[k] for k in ctx.konten_in("gezeichnetes_kapital"))
    persg_ind = sum(ctx.anzahl[k] for k in ctx.konten_in("privat"))
    ctx.kennzahlen["Rechtsform-Indizien (Kontenbild)"] = (
        f"KapG: {kapg_ind} Buchungen auf KSt-/gezeichnetes-Kapital-Konten · "
        f"PersG/EU: {persg_ind} Buchungen auf Privatkonten")
    out = []
    if angabe in ("", "unbekannt"):
        indikation = aus_name or (
            "kapitalgesellschaft" if kapg_ind and not persg_ind else
            "einzelunternehmen/personengesellschaft"
            if persg_ind and not kapg_ind else None)
        if indikation or kapg_ind or persg_ind:
            out.append(_b(ctx, "DQ-02", "mittel",
                f"Rechtsform nicht angegeben – Indikation aus "
                f"{'Mandantenname' if aus_name else 'Kontenbild'}: "
                f"{indikation or 'uneindeutig'}. Rechtsformabhängige Checks "
                "(PP-01/02/04, SB-10) laufen eingeschränkt.",
                "Rechtsform mit --rechtsform bzw. Parameter 'rechtsform' "
                "setzen, damit die passende Prüfbrille greift."))
        else:
            ctx.skip("DQ-02", "erfordert Rechtsform-Angabe oder auswertbare "
                              "Indizien (Mandantenname/Kontenbild)")
        return out
    if aus_name and aus_name != angabe:
        out.append(_b(ctx, "DQ-02", "hoch",
            f"Mandantenname '{ctx.mandant_name}' deutet auf "
            f"'{aus_name}', angegeben ist '{angabe}' – Auswertung mit "
            "falscher Rechtsform-Brille wäre ein gravierender Fehler.",
            "Rechtsform-Angabe bzw. Mandantenbezeichnung korrigieren und "
            "Lauf wiederholen."))
    if angabe in ("personengesellschaft", "einzelunternehmen") and kapg_ind:
        out.append(_b(ctx, "DQ-02", "hoch",
            f"{kapg_ind} Buchung(en) auf KSt-/gezeichnetes-Kapital-Konten "
            f"trotz Rechtsform '{angabe}'.",
            "Rechtsform-Angabe prüfen; Ausnahme: optierende "
            "Personengesellschaft (§ 1a KStG) – dann dokumentieren."))
    # Angabe KapG + Privatkonten-Buchungen: bewusst kein DQ-02 - deckt PP-04.
    return out
...
def sb_10_glatte_barbewegungen(ctx: Kontext) -> list[Befund]:
    from checks import _ist_kapitalgesellschaft
    if _ist_kapitalgesellschaft(ctx):
        ctx.skip("SB-10", "Rechtsform Kapitalgesellschaft: Privatkonten "
                          "unzulässig – siehe PP-04")
        return []
...
def bl_01_rap(ctx: Kontext) -> list[Befund]:
    out = []
    rap_bewegt = False
    for gruppe, art, empf in (
            ("rap_aktiv", "ARAP",
             "Auflösung der vorausgezahlten Aufwendungen bzw. Neubildung "
             "prüfen (§ 250 Abs. 1 HGB)."),
            ("rap_passiv", "PRAP",
             "Auflösung der vorab vereinnahmten Erträge bzw. Neubildung "
             "prüfen (§ 250 Abs. 2 HGB).")):
        for k in ctx.konten_in(gruppe):
            ...
            if abs(ctx.saldo[k]) > ctx.bagatelle:
                out.append(_b(ctx, "BL-01", "hinweis",
                    f"{art}-Konto {ctx.kontotext(k)} mit Bestand "
                    f"{eur(ctx.saldo[k])} EUR ohne jede unterjährige Bewegung "
                    f"(Vorjahres-Abgrenzung nicht aufgelöst?).",
                    empf, konto=k, betrag=ctx.saldo[k]))
    indikator = Decimal(str(ctx.param["rap_indikator_min_eur"]))
    typisch = sum((ctx.saldo_netto[k] for k in ctx.anzahl
                   if ctx.plan.in_gruppe(k, "abgrenzung_typisch")
                   and ctx.plan.ist_guv(k)), Decimal(0))
    if not rap_bewegt and typisch >= indikator:
        out.append(_b(ctx, "BL-01", "hinweis",
            f"Abgrenzungstypische Aufwendungen (Mieten, Versicherungen, "
...
def bl_02_rueckstellungen(ctx: Kontext) -> list[Befund]:
    ...
    for k in ctx.konten_in("rueckstellungen"):
        ...
        elif (unterjaehrig and ctx.susa_vj is not None
              and abs(ctx.saldo[k]) > ctx.bagatelle
              and ctx.saldo[k] == ctx.susa_vj.get(k)):
            out.append(_b(ctx, "BL-02", "hinweis",
                f"Rückstellungskonto {ctx.kontotext(k)}: Endbestand "
                f"{eur(ctx.saldo[k])} EUR trotz Bewegungen exakt wie im "
                "Vorjahr.",
                "Jährlich identische Rückstellungsbeträge deuten auf "
                "ungeprüfte Fortführung; Neubewertung zum Stichtag "
                "dokumentieren (§ 253 Abs. 1, 2 HGB).",
                konto=k, betrag=ctx.saldo[k]))
    return out


def bl_05_latente_steuern(ctx: Kontext) -> list[Befund]:
    if not ctx.plan.gruppen.get("latente_steuern"):
        ctx.skip("BL-05", "erfordert Konfiguration 'latente_steuern' "
                          "(Kontenbereiche des Kontenplans, § 274 HGB)")
        return []
    out = []
    for k in ctx.konten_in("latente_steuern"):
        if abs(ctx.saldo[k]) <= ctx.bagatelle and not ctx.bewegungen[k]:
            continue
        out.append(_b(ctx, "BL-05", "hinweis",
            f"Latente Steuern {ctx.kontotext(k)}: Saldo "
            f"{eur(ctx.saldo[k])} EUR.",
            "Bewertung prüfen: KSt-Satz sinkt ab VZ 2028 stufenweise von "
            "15 % auf 10 % ab 2032 (§ 23 Abs. 1 KStG i. d. F. des "
            "Investitionssofortprogramms v. 14.07.2025, BGBl. 2025 I "
            "Nr. 161); latente Steuern sind mit dem Steuersatz des "
            "erwarteten Umkehrjahres zu bewerten (§ 274 Abs. 2 HGB), "
            "bereits ab dem Abschluss 2025. Ansatzwahlrecht/Erleichterung "
            "§ 274a HGB beachten.",
            konto=k, betrag=ctx.saldo[k]))
    return out
...
def bl_04_ek_direktbuchungen(ctx: Kontext) -> list[Befund]:
    if ctx.wj_ende is None:
        return []
    abschluss_ab = ctx.wj_ende - timedelta(days=int(ctx.param["cutoff_fenster_vor_tage"]))
...
def et_01_geschenke(ctx: Kontext) -> list[Befund]:
    ...
    # Maßstab sind die Anschaffungskosten: mit Vorsteuerabzug netto, ohne
    # Vorsteuerabzug brutto (R 9b Abs. 2 EStR, Parameter vst_abzugsberechtigt).
    vst_abzug = bool(ctx.param.get("vst_abzugsberechtigt", True))
    massstab = "netto" if vst_abzug else "brutto"
    out = []
    for k in ctx.konten_in("geschenke_abz"):
        for d, _z, w, b in ctx.bewegungen[k]:
            if w <= 0 or ctx.ist_eb(b):
                continue
            wert = ctx.plan.netto(b) if vst_abzug else w
            if wert > grenze:
                out.append(_b(ctx, "ET-01", "mittel",
                    f"Geschenk {eur(wert)} EUR {massstab} auf Abziehbar-Konto "
                    f"{ctx.kontotext(k)} über der Grenze ({eur(grenze)} EUR "
                    f"je Empfänger/Jahr).",
                    "Umbuchung auf nicht abziehbar prüfen (§ 4 Abs. 5 Nr. 1 "
                    "EStG); Grenzbetrag je Empfänger und Jahr – Summierung "
                    "mehrerer Geschenke erfordert die Empfängeraufzeichnung "
                    "nach § 4 Abs. 7 EStG; Vorsteuerausschluss § 15 Abs. 1a "
                    "UStG beachten.",
                    konto=k, datum=d, betrag=wert, beleg=b.belegfeld1,
                    buchungstext=b.buchungstext, quelle=_q(b)))
    return out
...
def co_01_erloese_vor_wj_ende(ctx: Kontext) -> list[Befund]:
    """Erlös-/forderungsseitiger Cut-off: wesentliche Erlösbuchungen im
    Fenster vor dem WJ-Ende (Anker: DATEV-Header, nie 31.12. unterstellt).
    Die aufwands-/verbindlichkeitsseitige Spiegelprüfung nach dem WJ-Ende
    ist CO-02 (Folgejahres-Stapel)."""
    if ctx.wj_ende is None:
        ctx.skip("CO-01", "erfordert WJ-Ende (DATEV-Header: WJ-Beginn "
                          "bzw. Zeitraum)")
        return []
    tage = int(ctx.param["cutoff_fenster_vor_tage"])
    ab = ctx.wj_ende - timedelta(days=tage)
    ...
    for b in ctx.buchungen:
        ...
        if serien[(b.umsatz, b.konto, b.gegenkonto)] >= 4:
            continue  # Dauerbuchungen (Monatspauschalen etc.) sind kein Cut-off-Indiz
        guv = b.konto if ctx.plan.ist_guv(b.konto) else (
            b.gegenkonto if ctx.plan.ist_guv(b.gegenkonto) else None)
        if guv is None or not (ctx.plan.in_gruppe(guv, "ertrag")
                               or ctx.plan.in_gruppe(guv, "erloesschmaelerung")):
            continue
        if b.umsatz < minimum:
            continue
        if len(out) >= limit:
            uebrig += 1
            continue
        art = ("Erlösschmälerungs" if ctx.plan.in_gruppe(guv, "erloesschmaelerung")
               else "Erlös")
        out.append(_b(ctx, "CO-01", "hinweis",
            f"Wesentliche {art}buchung {eur(b.umsatz)} EUR am "
            f"{b.belegdatum:%d.%m.%Y} – im Cut-off-Fenster "
            f"{ab + timedelta(days=1):%d.%m.%Y}–{ctx.wj_ende:%d.%m.%Y} "
            f"vor dem WJ-Ende (Parameter 'cutoff_fenster_vor_tage' = "
            f"{tage}).",
            "Periodenzugehörigkeit prüfen (Umsatzrealisierung: "
            "Leistungsdatum, Abgrenzung, ggf. Stornierung im Folgejahr).",
            konto=guv, datum=b.belegdatum, betrag=b.umsatz,
            beleg=b.belegfeld1, buchungstext=b.buchungstext, quelle=_q(b),
            llm=True))
    _rest(ctx, out, "CO-01", uebrig, "Periodenend-Erlösbuchungen")
    return out


def co_02_aufwand_nach_wj_ende(ctx: Kontext) -> list[Befund]:
    """Aufwands-/verbindlichkeitsseitiger Cut-off (Vollständigkeit der
    Verbindlichkeiten): wesentliche Aufwandsbuchungen des Folgejahres im
    Fenster nach dem WJ-Ende. Fakultativ - Datenquelle ist der optionale
    Folgejahres-Stapel (--stapel-folgejahr); ohne Lieferung begründeter
    Skip, kein Mangel (der Prüfjahres-Stapel endet am WJ-Ende)."""
    if ctx.wj_ende is None:
        ctx.skip("CO-02", "erfordert WJ-Ende (DATEV-Header: WJ-Beginn "
                          "bzw. Zeitraum)")
        return []
    if not ctx.buchungen_folgejahr:
        ctx.skip("CO-02", "fakultativ – Folgejahres-Stapel nicht geliefert "
                          "(--stapel-folgejahr); Nachlauf-Prüfung entfällt "
                          "ohne Mangel")
        return []
    tage = int(ctx.param["cutoff_fenster_nach_tage"])
    fenster_von = ctx.wj_ende + timedelta(days=1)
    fenster_bis = ctx.wj_ende + timedelta(days=tage)
    fj_von = [i.datum_von or i.wj_beginn for i in ctx.infos_folgejahr
              if i.datum_von or i.wj_beginn]
    fj_bis = [i.datum_bis for i in ctx.infos_folgejahr if i.datum_bis]
    if fj_bis and max(fj_bis) <= ctx.wj_ende:
        ctx.skip("CO-02", f"Folgejahres-Stapel endet laut Header am "
                          f"{max(fj_bis):%d.%m.%Y} und liegt damit nicht "
                          f"nach dem WJ-Ende {ctx.wj_ende:%d.%m.%Y} – "
                          "Lieferung prüfen")
        return []
    if fj_von and min(fj_von) > fenster_von:
        ctx.skip("CO-02", f"Folgejahres-Stapel beginnt laut Header erst am "
                          f"{min(fj_von):%d.%m.%Y} – Cut-off-Fenster "
                          f"{fenster_von:%d.%m.%Y}–{fenster_bis:%d.%m.%Y} "
                          "nicht abgedeckt")
        return []
    minimum = Decimal(str(ctx.param["cutoff_min_eur"]))
    # Dauerbuchungs-Muster gegen das Prüfjahr erkennen (Januar-Miete usw.):
    # ein im Folgejahres-Fenster nur einmal vorkommender Satz kann dort
    # keine Serie bilden.
    serien = Counter((b.umsatz, b.konto, b.gegenkonto)
                     for b in ctx.buchungen if not ctx.ist_eb(b))
    out, uebrig = [], 0
    limit = _limit(ctx)
    for b in ctx.buchungen_folgejahr:
        if ctx.ist_eb(b) or b.belegdatum is None:
            continue
        if not (fenster_von <= b.belegdatum <= fenster_bis):
            continue
        if serien[(b.umsatz, b.konto, b.gegenkonto)] >= 4:
            continue
        guv = b.konto if ctx.plan.ist_guv(b.konto) else (
            b.gegenkonto if ctx.plan.ist_guv(b.gegenkonto) else None)
        if guv is None or not ctx.plan.in_gruppe(guv, "aufwand") \
                or ctx.plan.in_gruppe(guv, "afa"):
            continue
        if b.umsatz < minimum:
            continue
        if len(out) >= limit:
            uebrig += 1
            continue
        out.append(_b(ctx, "CO-02", "hinweis",
            f"Wesentliche Aufwandsbuchung {eur(b.umsatz)} EUR am "
            f"{b.belegdatum:%d.%m.%Y} im Folgejahr – im Cut-off-Fenster "
            f"{fenster_von:%d.%m.%Y}–{fenster_bis:%d.%m.%Y} nach dem "
            f"WJ-Ende (Parameter 'cutoff_fenster_nach_tage' = {tage}).",
            "Periodenzugehörigkeit prüfen (Vollständigkeit der "
            "Verbindlichkeiten: Leistungsdatum; ggf. Verbindlichkeit, "
            "Rückstellung oder RAP im Prüfjahr nachbuchen).",
            konto=guv, datum=b.belegdatum, betrag=b.umsatz,
            beleg=b.belegfeld1, buchungstext=b.buchungstext, quelle=_q(b),
            llm=True))
    _rest(ctx, out, "CO-02", uebrig, "Nachlauf-Aufwandsbuchungen")
    return out


# ------------------------------------------------- Personal/Privat (PP-04)

def pp_04_privat_bei_kapg(ctx: Kontext) -> list[Befund]:
    rechtsform = str(ctx.param.get("rechtsform", "unbekannt")).lower()
    if rechtsform in ("", "unbekannt"):
        ctx.skip("PP-04", "erfordert Rechtsform-Angabe (--rechtsform bzw. "
                          "Parameter 'rechtsform')")
        return []
    if rechtsform != "kapitalgesellschaft":
        ctx.skip("PP-04", f"Rechtsform '{rechtsform}': Privatkonten zulässig "
                          "(geprüft über PP-01/02, SB-10)")
        return []
    out = []
    for k in ctx.konten_in("privat"):
        bew = [e for e in ctx.bewegungen[k] if not ctx.ist_eb(e[3])]
        if not bew:
            continue
        summe = sum((abs(w) for _d, _z, w, _b_ in bew), Decimal(0))
        out.append(_b(ctx, "PP-04", "mittel",
            f"Privatkonto {ctx.kontotext(k)} bei Kapitalgesellschaft bebucht "
            f"({len(bew)} Buchung(en), Volumen {eur(summe)} EUR).",
            "Auf Gesellschafter-Verrechnungskonto umgliedern; vGA-Risiko "
            "würdigen (§ 8 Abs. 3 Satz 2 KStG).",
            konto=k, betrag=summe))
    return out
